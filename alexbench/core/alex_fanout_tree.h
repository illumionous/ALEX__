  ...
  for (int i = 0; i < fanout; i++) {
    ...
    if (left_boundary == right_boundary) {
      used_fanout_tree_nodes.push_back(
          {level, i, 0, left_boundary, right_boundary, false, 0, 0, 0, 0, 1, 0});
      continue;
    }
...
std::pair<int, double> find_best_fanout_bottom_up(
  ...
  fanout_tree.push_back(
      {{0, 0, best_cost, 0, num_keys, false, 0, 0, 0, 0, 1, num_keys}});
  ...
  std::cout<<"best is "<<best_level<<"  "<<best_cost<<std::endl;
...
std::pair<int, double> find_best_fanout_top_down(
  ...
  while (true) {
    ...
    for (FTNode& tree_node : fanout_tree[fanout_tree_level - 1]) {
      ...
      LinearModel_sig<T> node_models[2];
      for (int i = 0; i < 2; i++) {
    ...
    for (int i = 0; i < fanout; i++) {
      ...
      if (left_boundary == right_boundary) {
        new_level.push_back({fanout_tree_level, i, 0, left_boundary,
                             right_boundary, false, 0, 0, 0, 0,1, 0});
        continue;
      }
