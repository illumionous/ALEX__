...
// node's parent.
 ...
 public:
  ...
  void bulk_load(const V values[], int num_keys) {
    ...
    double capacity=num_keys/data_node_type::kInitDensity_;
    root_node_->model_.c_ = capacity/ static_cast<double>(max_key-min_key);
    // Compute cost of root node
    LinearModel_sig<T> root_data_node_model;
 ...
 private:
  ...
  void bulk_load_node(const V values[], int num_keys, AlexNode<T, P>*& node,
                      int total_keys,
                      const LinearModel_sig<T>* data_node_model = nullptr) {
    ...
    if (best_fanout_tree_cost < node->cost_ ||
      ...
      for (fanout_tree::FTNode& tree_node : used_fanout_tree_nodes) {
        ...
        child_node->model_.c_ =child_node->num_children_/(right_boundary-left_boundary);
        model_node->children_[cur] = child_node;
        LinearModel_sig<T> child_data_node_model(tree_node.a, tree_node.b,tree_node.c);
  ...
  data_node_type* bulk_load_leaf_node_from_existing(
    ...
    if (tree_node) {
      ...
      // recompute it
      LinearModel_sig<T> precomputed_model(tree_node->a, tree_node->b,tree_node->c);
      ...
      LinearModel_sig<T> precomputed_model(existing_node->model_);
 ...
 public:
  ...
  P* get_payload(const T& key) const {
    ...
    data_node_type* leaf = get_leaf(key);
    // if(key>100&&key<=101){
    //   std::vector<TraversalNode>* c;
    //   data_node_type* leaf = get_leaf(key,c);
    //   for (auto it = c->begin(); it != c->end(); ++it) {
    //     std::cout << (*it).bucketID<< " ";
    //   }
    // }
    int idx = leaf->find_key(key);
    if (idx < 0) {
