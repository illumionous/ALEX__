...
class AlexDataNode : public AlexNode<T, P> {
 public:
  ...
  static double compute_expected_cost(
      ...
      const LinearModel_sig<T>* existing_model = nullptr, bool use_sampling = false,
  ...
  static double compute_expected_cost_sampling(
      const V* values, int num_keys, double density,
      double expected_insert_frac,
      const LinearModel_sig<T>* existing_model = nullptr,
      DataNodeStats* stats = nullptr) {
    ...
    LinearModel_sig<T> model;  // trained for full dense array
    if (existing_model == nullptr) {
      build_model(values, num_keys, &model);
    } else {
      model.a_ = existing_model->a_;
    ...
    while (true) {
      ...
      LinearModel_sig<T> sample_model(model.a_, model.b_,model.c_);
      ...
      if (expected_insert_frac == 0) {
        ...
        SampleDataNodeStats tmp_stats;
        tmp_stats.log2_sample_size = std::log2(sample_num_keys);
        tmp_stats.num_search_iterations = acc.get_expected_num_search_iterations();
        tmp_stats.log2_num_shifts = std::log2(acc.get_expected_num_shifts());
        sample_stats.push_back(tmp_stats);
      }
  ...
  static void build_node_implicit_sampling(const V* values, int num_keys,
                                           ...
                                           const LinearModel_sig<T>* sample_model) {
    int last_position = -1;
  ...
  static double compute_expected_cost_from_existing(
    ...
    if (existing_model == nullptr) {
      ...
      LinearModelBuilder_sig<T> builder(&model);
  ...
  void bulk_load(const V values[], int num_keys,
                 const LinearModel_sig<T>* pretrained_model = nullptr,
  ...
  void bulk_load_from_existing(
      ...
      const LinearModel_sig<T>* precomputed_model = nullptr,
    ...
    if (precomputed_model == nullptr || precomputed_num_actual_keys == -1) {
      const_iterator_type it(node, left);
      LinearModelBuilder_sig<T> builder(&(this->model_));
      for (int i = 0; it.cur_idx_ < right && !it.is_end(); it++, i++) {
        builder.add(it.key(), i);
        num_actual_keys++;
      }
  ...
  static void build_model(const V* values, int num_keys, LinearModel_sig<T>* model,
