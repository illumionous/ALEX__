...
class GaussianRBFModel {
 public:
  std::vector<std::pair<T, double>> centers_;  // RBF centers
  std::vector<double> weights_;  // RBF weights
  double gamma_;  // Gaussian RBF parameter

  GaussianRBFModel() = default;
  explicit GaussianRBFModel(double gamma) : gamma_(gamma) {}
  explicit GaussianRBFModel(const GaussianRBFModel& other)
      : centers_(other.centers_), weights_(other.weights_), gamma_(other.gamma_) {}

  inline double gaussian_rbf(const T& x, const T& center) const {
    double diff = static_cast<double>(x) - static_cast<double>(center);
    return std::exp(-gamma_ * diff * diff);
  }

  inline int predict(T key) const {
    double prediction = 0.0;
    for (size_t i = 0; i < centers_.size(); ++i) {
      prediction += weights_[i] * gaussian_rbf(key, centers_[i].first);
    }
...
template <class T>
class LinearModel_sig {
 public:
  double a_ = 0;  // slope
  double b_ = 0;  // intercept
  double c_ = 1;  // n^2param
  double data_base_ = 0; 

  LinearModel_sig() = default;
  LinearModel_sig(double a, double b, double c) : a_(a), b_(b), c_(c) {}
  explicit LinearModel_sig(const LinearModel_sig& other)
  ...
  inline double sigmoid(double x) const {
    return 1.0 / (1.0 + std::exp(-c_ * x));
  }

  inline int predict(T key ) const {
    double linear_output = a_ * static_cast<double>(key) + b_;
    return static_cast<int>(sigmoid(linear_output) * data_base_);
  }
...
template <class T>
class LinearModelBuilder_sig {
 public:
  LinearModel_sig<T>* model_;

  explicit LinearModelBuilder_sig<T>(LinearModel_sig<T>* model) : model_(model) {}
  ...
  void compute_sigmoid_parameter() {
  // 计算均值
  double x_mean = static_cast<double>(x_sum_) / count_;
  double y_mean = static_cast<double>(y_sum_) / count_;

  
  double x_var = std::max(static_cast<double>(xx_sum_ / count_ - x_mean * x_mean), 1e-10);
  double y_var = std::max(static_cast<double>(yy_sum_ / count_ - y_mean * y_mean), 1e-10);

  // 计算协方差
  double xy_covar = xy_sum_ / count_ - x_mean * y_mean;

  // 计算Sigmoid参数c
  double c = (xy_covar / x_var) / (y_var / x_var);
  model_->c_ = c;
}
  void build() {
    if (count_ <= 1) {
      ...
      compute_sigmoid_parameter();
      model_->data_base_ = static_cast<double>(count_) / 0.8;
      return;
    }
...
constexpr double kNodeLookupsWeight = 40;
