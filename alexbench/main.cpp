...
int main(int argc, char* argv[]) {
  ...
  while (true) {
    ...
    alex::Alex<double, double>::NodeIterator node_it(&index);
    int num=0;
    ...
    std::cout<<"before insert datanode number is "<<num<<std::endl;
    ...
    //test for rootnode information
    std::cout<<" after inserts now is time to know how many datanodes here"<<index.stats_.num_data_nodes<< std::endl;
    std::cout<<"now is time to know the rootmodel thing"<<std::endl;
    
    
    std::cout<<"num_model_nodes are"<<index.stats_.num_model_nodes<<"  "<<"num_data_nodes"<<index.stats_.num_data_nodes<<std::endl;
    std::cout<<"num_model_node_splits are"<<index.stats_.num_model_node_splits<<"  "<<std::endl;
    alex::Alex<double, double>::NodeIterator node_it1(&index);
    int num1=0;
    for (; !node_it1.is_end(); node_it1.next()) {
      auto node = node_it1.current();
      if (node->is_leaf_) {
        num1++;
        if(num1%8000==0){
        auto datanode = static_cast<alex::AlexDataNode<double, double>*>(node);
        std::cout<<"node number is "<<num1<<std::endl;
        std::cout<<"nodes shifts is "<<datanode->num_shifts_<<std::endl;
        std::cout<<"nodes lookup is "<<datanode->num_lookups_<<std::endl;
        std::cout<<"nodes num_resizes_ is "<<datanode->num_resizes_<<std::endl; 
        std::cout<<"nodes num_keys_ is "<<datanode->num_keys_<<std::endl; 
        std::cout<<"nodes num_capacity_ is "<<datanode->data_capacity_<<std::endl; 
        std::cout<<"the fill rate is  "<<datanode->num_keys_ /  datanode->data_capacity_<<std::endl;
        std::cout<<"nodes out of bound inserts are "<<datanode->num_right_out_of_bounds_inserts_<<"ss" <<datanode->num_left_out_of_bounds_inserts_<<std::endl; 
    ...
    std::cout<<"after insert datanode number is "<<num1<<std::endl;
